...
public class LivroSpecs {
    ...
    public static Specification<Livro> autorNomeLike(String nomeAutor) {
        return (root, query, cb) -> cb.like(cb.lower(root.join("autor").get("nome")), "%" + nomeAutor.toLowerCase() + "%");
    }
    ...
    public static Specification<Livro> anoPublicacaoEquals(Integer anoPublicacao) {
        return (root, query, cb) -> cb.equal(cb.function("to_char", String.class, root.get("dtPublicacao"), cb.literal("YYYY")),anoPublicacao.toString());
    }
